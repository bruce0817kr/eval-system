...
export async function POST(
  ...
  try {
    ...
    const octomoResult = await verifyOtpViaOctomo(normalizedPhone, parsed.data.otpCode)
    const localResult = await verifyOtp(normalizedPhone, parsed.data.otpCode)
    const otpVerified = octomoResult || localResult
